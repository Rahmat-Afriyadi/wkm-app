...
import ButttonExportDataPembayaranRenewal from "@/components/form/data-renewal/form-export-pembayaran";
  ...
  content = (
    <>
      <div className="grid gap-6 mb-6 md:grid-cols-2">
        ...
        <ButttonExportDataPembayaranRenewal
          params={{
            tgl1: value.startDate,
            tgl2: value.endDate,
          }}
        />
      </div>
      ...
      {/* <div className="grid grid-cols-1 gap-5 mt-5 sm:grid-cols-3">
        {stats.map((item) => (
          <div key={item.id} className="relative px-4 py-5 overflow-hidden bg-white rounded-lg shadow sm:pt-6 sm:px-6">
            <dt>
              <div className=" absolute p-3 rounded-md bg-yellow">
                <item.icon className="w-6 h-6 text-black" aria-hidden="true" />
              </div>
        ...
        <div className="sm:flex-auto">
          <div className="flex flex-col lg:flex-row gap-3 lg:gap-0 lg:items-end">
            <div className="max-w-xs mr-2 w-80">
              <Search id="search-query" name="search_query" placeholder={"Search for a asuransi..."} />
            </div>
          </div>
        </div>
