...
const ItemTicketQueue = ({ ticket, id }) => {
  ...
  return (
    <tr className={id % 2 === 0 ? "bg-gray-50" : "hover:bg-gray-100"}>
      ...
      <td className="px-3 py-4 text-sm">{ticket.assign_date ? new Date(ticket.assign_date).toLocaleDateString() : "Belum Ditetapkan"}</td>
      <td className="px-3 py-4 text-sm whitespace-nowrap">
        {/* Edit button */}
        <span
          className="text-blue-600 cursor-pointer"
          onClick={() => router.push("/tiket-bantuan/detail/" + ticket.no_ticket)} // Call the edit function with the ticket ID
        >
