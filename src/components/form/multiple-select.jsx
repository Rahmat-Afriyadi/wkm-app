...
export default function MultipleSelect({ selected, setSelected, dataKerja }) {
  ...
  const filteredTags = dataKerjaList.filter(
    (item) => item[1]?.toLocaleLowerCase()?.includes(query.toLocaleLowerCase()?.trim()) && !selected.includes(item[0])
  );

  const isDisable =
    !query?.trim() ||
    selected.filter((item) => item[1]?.toLocaleLowerCase()?.trim() === query?.toLocaleLowerCase()?.trim())?.length;
            ...
            return (
              ...
              >
                ...
                <div
                  ...
                  onClick={() => setSelected(selected.filter((i) => i[0] !== tag[0]))}
                >
                  X
                </div>
              </div>
