...
export async function readManyApproval(query) {
  const { search, limit, pageParams, tgl1, tgl2, sb, ja } = query;

  const response = await AuthGetApi(
    "/asuransi/master-approval?" +
      new URLSearchParams({
        search,
        sb,
        ja,
  ...
  const resultLength = await AuthGetApi(
    "/asuransi/master-approval-count?" +
      new URLSearchParams({
        search,
        sb,
        ja,
...
export async function rekapAsuransiByStatusKdUser(query) {
  const { tgl1, tgl2 } = query;
  const response = await AuthGetApi(
    "/asuransi/rekap-by-status-kduser?" +
      new URLSearchParams({
        tgl1,
        tgl2,
      })
